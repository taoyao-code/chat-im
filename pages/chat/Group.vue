	...
	<view>
		<view class="cu-chat">
			...
			<scroll-view :scroll-y="true" class="scroll-y" :scroll-top="scrollTop" :scroll-with-animation="true">
				<div class="chatBox">
					<view class="cu-item" v-for="(item,index) in groupChat" :key="index" :class="{'self':item.userid==selfName}">
						...
						<view class="cu-avatar radius" v-if="item.userid==selfName" style="background-image:url(https://ossweb-img.qq.com/images/lol/web201310/skin/big107000.jpg);"></view>
						<view class="date">{{item.userid}}</view> <!-- 昵称 -->
					</view>
				</div>
			</scroll-view>
		...
		<view class="cu-bar foot input" id="foot" :style="[{bottom:InputBottom+'px'}]" :class="{'showVoice':showVoice}">
			...
			<input class="solid-bottom" :adjust-position="false" :focus="false" maxlength="300"  @focus="InputFocus"
...
<script>
	...
	export default {
		...
		onLoad(option) {
			this.selfName = uni.getStorageSync('UID');
		...
		methods: {
			sendMsg({Msg = this.userMsg,type = 1} = {}) {
				if (Msg == '') {
					return false;
				}
				let msg = {
